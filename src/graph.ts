...
export function inspectGraph(
  ...
  function identify(target: Resolvable | DependencyIdentity): number {
    ...
    const node: GraphNode = {
      ...
      kind: isToken(target) ? 'token' : isRef(target) ? 'ref' : 'dependency',
    };
    if (!isToken(target)) {
      node.lifetime =
        getDefinition(isRef(target) ? target.dependency : target).options.lifetime ?? 'singleton';
    }
    if (isRef(target)) node.params = target.params;
    nodes.push(node);
    return id;
  }
  ...
  function visit(target: Resolvable, path: readonly string[]): number {
    ...
    if (expanded.has(target)) return id;
    expanded.add(target);
    ...
    if (isToken(target)) {
      ...
      if ('dependency' in binding) {
        ...
        edges.push({ from: id, to: visit(binding.dependency, nextPath), kind: 'dependency' });
      }
      return id;
    }
  ...
  function check(id: number, path: string[]): void {
    ...
    active.add(id);
    for (const child of adjacency.get(id) ?? []) check(child, [...path, nodes[id]!.name]);
    active.delete(id);
    visited.add(id);
  }
  for (const node of nodes) check(node.id, []);
  return { nodes, edges };
}
